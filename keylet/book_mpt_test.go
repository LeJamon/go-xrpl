...
// TestBookBaseIssueMatchesBookDir asserts BookBase preserves the existing
// Issue/Issue book prefix and clears its quality bytes.
func TestBookBaseIssueMatchesBookDir(t *testing.T) {
	...
	want := Quality(BookDir(paysCur, paysIss, getsCur, getsIss), 0)
	...
	wantD := Quality(BookDirWithDomain(paysCur, paysIss, getsCur, getsIss, domain), 0)
...
func TestBookBaseMPTFieldLayout(t *testing.T) {
	...
	expect := func(fields ...[]byte) [32]byte {
		key := indexHash(spaceBookDir, fields...)
		clear(key[24:])
		return key
	}
...
func TestBookBaseQualityIsZero(t *testing.T) {
	paysMPT := MakeMPTID(7, [20]byte{7, 7, 7})
	getsMPT := MakeMPTID(8, [20]byte{8, 8, 8})
	key := BookBase(MPTSide(paysMPT), MPTSide(getsMPT), nil).Key
	for _, value := range key[24:] {
		if value != 0 {
			t.Fatalf("quality bytes = %x, want 0000000000000000", key[24:])
		}
	}
}
