...
static_assert(SECP256K1_COMPACT_STANDARD_PROOF_SIZE == 192, "send sigma proof size");
static_assert(kMPT_DOUBLE_BULLETPROOF_SIZE == 754, "send range proof size");
static_assert(SECP256K1_COMPACT_STANDARD_PROOF_SIZE + kMPT_DOUBLE_BULLETPROOF_SIZE == 946, "send proof size");
static_assert(SECP256K1_COMPACT_CLAWBACK_PROOF_SIZE == 64, "clawback proof size");
static_assert(SECP256K1_COMPACT_CONVERTBACK_PROOF_SIZE + kMPT_SINGLE_BULLETPROOF_SIZE == 816, "convert back proof size");

static std::atomic<bool> force_unavailable{false};

static const secp256k1_context* mpt_context() {
    return force_unavailable.load(std::memory_order_relaxed) ? nullptr : mpt_secp256k1_context();
}

extern "C" int go_mpt_available(void) { return mpt_context() != nullptr; }

extern "C" void go_mpt_test_force_unavailable(int unavailable) {
    force_unavailable.store(unavailable != 0, std::memory_order_relaxed);
}
...
extern "C" int go_mpt_valid_ciphertext(const uint8_t ciphertext[66]) {
    if (mpt_context() == nullptr) return 0;
...
extern "C" int go_mpt_canonical_zero(const uint8_t pubkey[33], const uint8_t account[20], const uint8_t issuance[24], uint8_t out[66]) {
    ...
    if (secp256k1_ec_pubkey_parse(ctx, &key, pubkey, 33) != 1) return 0;
    if (generate_canonical_encrypted_zero(ctx, &c1, &c2, &key, account, issuance) != 1) return 0;
...
extern "C" int go_mpt_send_context(const uint8_t acc[20], const uint8_t issuance[24], uint32_t sequence, const uint8_t destination[20], uint32_t version, uint8_t out[32]) {
    if (mpt_context() == nullptr) return 0;
    return mpt_get_send_context_hash(account(acc), issuance_id(issuance), sequence, account(destination), version, out) == 0;
}

extern "C" int go_mpt_clawback_context(const uint8_t acc[20], const uint8_t issuance[24], uint32_t sequence, const uint8_t holder[20], uint8_t out[32]) {
    if (mpt_context() == nullptr) return 0;
    return mpt_get_clawback_context_hash(account(acc), issuance_id(issuance), sequence, account(holder), out) == 0;
}
...
extern "C" int go_mpt_verify_send(const uint8_t proof[946], const uint8_t pubs[132], const uint8_t ciphertexts[264], uint8_t participant_count, const uint8_t spending[66], const uint8_t amount_commitment[33], const uint8_t balance_commitment[33], const uint8_t context[32]) {
    if (mpt_context() == nullptr) return 0;
    if (participant_count != 3 && participant_count != 4) return 0;
    ...
    return mpt_verify_send_proof(proof, participants, participant_count, spending, amount_commitment, balance_commitment, context) == 0;
}

extern "C" int go_mpt_verify_clawback(const uint8_t proof[64], uint64_t amount, const uint8_t pubkey[33], const uint8_t ciphertext[66], const uint8_t context[32]) {
    if (mpt_context() == nullptr) return 0;
    return mpt_verify_clawback_proof(proof, amount, pubkey, ciphertext, context) == 0;
}

extern "C" int go_mpt_rerandomize(const uint8_t ciphertext[66], const uint8_t pubkey[33], const uint8_t randomness[32], uint8_t out[66]) {
    if (mpt_context() == nullptr) return 0;
    uint8_t zero[66];
    if (mpt_encrypt_amount(0, pubkey, randomness, zero) != 0) return 0;
...
extern "C" int go_mpt_generate_send_proof(const uint8_t private_key[32], uint64_t amount, const uint8_t pubs[132], const uint8_t ciphertexts[264], uint8_t participant_count, const uint8_t transaction_blind[32], const uint8_t context[32], const uint8_t amount_commitment[33], const uint8_t balance_commitment[33], uint64_t balance, const uint8_t spending[66], const uint8_t balance_blind[32], uint8_t proof[946]) {
    ...
    return mpt_get_confidential_send_proof(private_key, participants[0].pubkey, amount, participants, participant_count, transaction_blind, context, amount_commitment, &balance_params, proof, &proof_len) == 0 && proof_len == 946;
}

extern "C" int go_mpt_generate_clawback_proof(const uint8_t private_key[32], const uint8_t public_key[33], const uint8_t context[32], uint64_t amount, const uint8_t ciphertext[66], uint8_t proof[64]) {
    if (mpt_context() == nullptr) return 0;
    return mpt_get_clawback_proof(private_key, public_key, context, amount, ciphertext, proof) == 0;
}
