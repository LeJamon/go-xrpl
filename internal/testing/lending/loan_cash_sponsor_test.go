...
func testLoanAccountingWithSponsor(t *testing.T, cash, cleanup bool, mode string) {
	...
	prepare := func(transaction tx.Transaction, fee string, reserve bool) {
		...
		if flags != 0 {
			...
			if !prefunded {
				signature, err := txsign.SignSponsor(transaction, feeSponsor.PublicKeyHex(), "00"+feeSponsor.PrivateKeyHex())
				require.NoError(t, err)
	...
	counterpartySignature, err := txsign.SignCounterparty(loanSet, owner.PublicKeyHex(), "00"+owner.PrivateKeyHex())
	require.NoError(t, err)
