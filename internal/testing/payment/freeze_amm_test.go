...
func TestFreeze_AMMWhenFrozen(t *testing.T) {
	setLineFreeze := func(t *testing.T, env *jtx.TestEnv, issuer, holder *jtx.Account, flags uint32) {
		t.Helper()
		ts := trustset.TrustLine(issuer, "USD", holder, "0").BuildTrustSet()
		ts.SetFlags(flags)
		jtx.RequireTxSuccess(t, env.Submit(ts))
		env.Close()
	}
	...
	run := func(t *testing.T, deepFreeze bool) {
		...
		env.FundAmount(G1, uint64(jtx.XRP(10000)))
		env.FundAmount(A1, uint64(jtx.XRP(10000)))
		env.FundAmount(A2, uint64(jtx.XRP(10000)))
		env.Close()

		jtx.RequireTxSuccess(t, env.Submit(trustset.TrustLine(A1, "USD", G1, "10000").Build()))
		jtx.RequireTxSuccess(t, env.Submit(trustset.TrustLine(A2, "USD", G1, "10000").Build()))
		env.Close()

		usd1000 := tx.NewIssuedAmountFromFloat64(1000, "USD", G1.Address)
		jtx.RequireTxSuccess(t, env.Submit(paytest.PayIssued(G1, A1, usd1000).Build()))
		jtx.RequireTxSuccess(t, env.Submit(paytest.PayIssued(G1, A2, usd1000).Build()))
		env.Close()

		// G1 creates an AMM with XRP(1000) and USD(1000).
		result := env.Submit(ammtest.AMMCreate(G1,
			tx.NewXRPAmount(jtx.XRP(1000)), usd1000).Build())
		...
		xrp11 := tx.NewXRPAmount(jtx.XRP(11))
		...
		result = env.Submit(paytest.Pay(A1, A2, uint64(jtx.XRP(10))).
		...
		result = env.Submit(paytest.Pay(A2, A1, uint64(jtx.XRP(10))).
			PathsCurrency("XRP", nil).
			SendMax(usd11).
			NoDirectRipple().
			Build())
