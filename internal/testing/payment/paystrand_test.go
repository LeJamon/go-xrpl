...
func trust(account, issuer *jtx.Account, currency string, limit float64) tx.Transaction {
	return trustset.TrustLine(account, currency, issuer, fmt.Sprintf("%f", limit)).Build()
}

// PathStep helpers for creating path elements
func accountPath(acc *jtx.Account) payment.PathStep {
...
func issuePath(currency string, issuer *jtx.Account) payment.PathStep {
	return payment.PathStep{Currency: currency, Issuer: issuer.Address}
}

func issuerPath(issuer *jtx.Account) payment.PathStep {
...
func TestToStrand_XRPtoXRPWithPath(t *testing.T) {
	...
	result := env.Submit(Pay(alice, bob, uint64(jtx.XRP(100))).Paths(paths).Build())
	jtx.RequireTxFail(t, result, jtx.TemBAD_SEND_XRP_PATHS)
}
...
func TestToStrand_NoTrustLine(t *testing.T) {
	...
	jtx.RequireTxFail(t, result, jtx.TecPATH_DRY)
}
...
func TestRIPD1373_SendXRPPaths(t *testing.T) {
	...
	result = env.Submit(Pay(alice, carol, uint64(jtx.XRP(100))).Paths(paths).NoDirectRipple().Build())
	jtx.RequireTxFail(t, result, "temBAD_SEND_XRP_PATHS")
}
...
func TestRIPD1373_SendXRPMax(t *testing.T) {
	...
	result = env.Submit(Pay(alice, carol, uint64(jtx.XRP(100))).SendMax(xrpMax).Paths(paths).NoDirectRipple().Build())
	jtx.RequireTxFail(t, result, "temBAD_SEND_XRP_MAX")
}
...
func TestNoAccount_NoDst(t *testing.T) {
	...
	result := env.Submit(Pay(alice, unfunded, uint64(jtx.XRP(1))).Build())
...
func TestPaymentSandbox_SelfFunding(t *testing.T) {
	...
	env.FundAmount(sender, uint64(jtx.XRP(10000)))
	env.FundAmount(receiver, uint64(jtx.XRP(10000)))
...
func TestToStrand_LastStepXRPFromOffer(t *testing.T) {
	...
	paths := [][]payment.PathStep{{currencyPath("XRP")}}
	result = env.Submit(Pay(alice, bob, uint64(jtx.XRP(10))).
		SendMax(usdSendMax).
		Paths(paths).
		Build())
	jtx.RequireTxSuccess(t, result)
...
func TestPaymentSandbox_Reserve(t *testing.T) {
	...
	env.FundAmount(alice, reserve+uint64(jtx.XRP(1)))
	env.Close()
