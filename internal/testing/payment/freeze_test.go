...
func TestFreeze_GlobalFreeze(t *testing.T) {
	...
	env.FundAmount(gw, uint64(jtx.XRP(12000)))
	env.FundAmount(alice, uint64(jtx.XRP(1000)))
	env.FundAmount(bob, uint64(jtx.XRP(20000)))
	env.Close()
	...
	// Payments through A2's offer should fail because the offer is unfunded (frozen output side)
	{
		...
		result = env.Submit(
			PayIssued(A1, G1, usd10).
				SendMax(tx.NewXRPAmount(jtx.XRP(11))).
...
func TestFreeze_OffersWhenFrozen(t *testing.T) {
	...
	env.FundAmount(G1, uint64(jtx.XRP(1000)))
	env.FundAmount(A2, uint64(jtx.XRP(2000)))
	env.FundAmount(A3, uint64(jtx.XRP(1000)))
	env.FundAmount(A4, uint64(jtx.XRP(1000)))
	env.Close()
	...
	xrp999 := tx.NewXRPAmount(jtx.XRP(999))
	...
	xrpOffer := tx.NewXRPAmount(jtx.XRP(999))
