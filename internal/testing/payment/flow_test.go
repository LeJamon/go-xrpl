...
func readOffer(t *testing.T, env *jtx.TestEnv, acc *jtx.Account, seq uint32) *state.LedgerOffer {
	t.Helper()
...
func TestFlow_DirectStep(t *testing.T) {
	t.Run("XRP transfer", func(t *testing.T) {
		...
		require.GreaterOrEqual(t, bobBalance, uint64(jtx.XRP(10100)),
...
func TestFlow_LimitQuality(t *testing.T) {
	...
	xrpMax := tx.NewXRPAmount(jtx.XRP(100))
	paths := [][]payment.PathStep{{
		issuePath("USD", gw),
	}}
...
func TestFlow_SelfPayment2(t *testing.T) {
	...
	jtx.RequireIOUBalance(t, env, alice, gw1, "USD", 500)
	jtx.RequireIOUBalance(t, env, alice, gw2, "EUR", 600)
	jtx.RequireOffers(t, env, alice, 1)
...
func TestFlow_SelfFundedXRPEndpoint(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			xrp50k := tx.NewXRPAmount(jtx.XRP(50000))
			...
			sendMaxXRP := tx.NewXRPAmount(jtx.XRP(20000))
			paths := [][]payment.PathStep{{
				issuePath("USD", gw),
			}}
...
func TestFlow_ReexecuteDirectStep(t *testing.T) {
	...
	// partial payment, no direct ripple
	xrp10000 := tx.NewXRPAmount(jtx.XRP(10000))
	usd100 := tx.NewIssuedAmountFromFloat64(100, "USD", gw.Address)
	paths := [][]payment.PathStep{{
		currencyPath("XRP"),
	}}
	payTx := Pay(alice, bob, uint64(jtx.XRP(10000))).
		Paths(paths).
		SendMax(usd100).
		NoDirectRipple().
		PartialPayment().
...
func TestFlow_RIPD1443(t *testing.T) {
	t.Run("CrossIssuerPathDry", func(t *testing.T) {
		...
		env.FundAmountNoRipple(bob, uint64(jtx.XRP(100000000)))
		...
		xrp1 := tx.NewXRPAmount(jtx.XRP(1))
		...
		payTx1 := Pay(alice, alice, uint64(jtx.XRP(1))).
			Paths(paths1).
			SendMax(gwUsd1000).
			NoDirectRipple().
			Build()
	...
	t.Run("NoRippleBlocksReversePathDry", func(t *testing.T) {
		...
		gwUsd1000 := tx.NewIssuedAmountFromFloat64(1000, "USD", gw.Address)
		xrpMax := tx.NewXRPAmount(jtx.XRP(100000))
...
func TestFlow_CircularXRP(t *testing.T) {
	...
	// Verify balances
	jtx.RequireIOUBalance(t, env, alice, gw, "USD", 90)
...
func TestFlow_PaymentWithTicket(t *testing.T) {
	...
	jtx.WithTicketSeq(payTx, ticketSeq)
	result := env.Submit(payTx)
	jtx.RequireTxSuccess(t, result)
	env.Close()
