...
func TestConfidentialSendBatchStaleProofModes(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			batchFee := uint64(220)
			batch := batchtest.NewBatchBuilder(fixture.sender, outerSequence, batchFee, test.flag).AddInnerTx(first).AddInnerTx(second).MustBuild()
...
func TestConfidentialClawbackBatchAndFeeDispatch(t *testing.T) {
	...
	batchFee := uint64(130)
	batch := batchtest.NewBatchBuilder(fixture.issuer, outerSequence, batchFee, batchtx.BatchFlagAllOrNothing).AddInnerTx(clawback).AddInnerTx(payment).MustBuild()
	require.Equal(t, batchFee, batch.CalculateMinimumFee(fixture.env.Ledger(), tx.EngineConfig{BaseFee: fixture.env.BaseFee()}))
	result := fixture.env.Submit(batch)
	jtx.RequireTxSuccess(t, result)
	fixture.env.Close()
