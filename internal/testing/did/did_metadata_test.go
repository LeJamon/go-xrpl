...
func TestDIDMetadataLifecycle(t *testing.T) {
	...
	create := env.Submit(did.DIDSet(alice).URI("uri1").Document("doc1").Data("data1").Build())
	...
	require.Equal(t, hex.EncodeToString([]byte("data1")), strings.ToLower(createDID.NewFields["Data"].(string)))
	...
	update := env.Submit(did.DIDSet(alice).URI("uri2").Document("").Data("data2").Build())
	...
	require.Equal(t, hex.EncodeToString([]byte("data1")), strings.ToLower(updateDID.PreviousFields["Data"].(string)))
	require.Equal(t, hex.EncodeToString([]byte("uri2")), strings.ToLower(updateDID.FinalFields["URI"].(string)))
	require.Equal(t, hex.EncodeToString([]byte("data2")), strings.ToLower(updateDID.FinalFields["Data"].(string)))
	...
	checkVL(t, "Data", updated.Data, "data2")
	...
	require.Equal(t, hex.EncodeToString([]byte("data2")), strings.ToLower(deletedDID.FinalFields["Data"].(string)))
