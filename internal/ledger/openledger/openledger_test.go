...
// TestOpenLedger_CurrentTxsSeq_AgreesWithSlice verifies the iter.Seq variant
// of CurrentTxs yields the same blobs in the same order as the legacy slice
// API and supports early termination via yield-false.
func TestOpenLedger_CurrentTxsSeq_AgreesWithSlice(t *testing.T) {
	env := testenv.NewTestEnv(t)
	env.SetVerifySignatures(true)
	...
	senders := make([]*testenv.Account, N)
	for i := 0; i < N; i++ {
		senders[i] = testenv.NewAccount("seq-sender-" + itoa(i))
	}
	dest := testenv.NewAccount("seq-dest")
	all := append([]*testenv.Account{dest}, senders...)
	env.Fund(all...)
	parent := closedParent(t, env)
	...
	for i := 0; i < N; i++ {
		...
		if _, res := ol.Submit(pt, cfg, nil); res != openledger.ResultSuccess {
			t.Fatalf("Submit[%d] result=%v", i, res)
		}
	}

	slice := ol.CurrentTxs()
	var fromSeq [][]byte
	for blob := range ol.CurrentTxsSeq() {
		fromSeq = append(fromSeq, blob)
	}
	if len(slice) != len(fromSeq) {
		t.Fatalf("CurrentTxsSeq len = %d, CurrentTxs len = %d", len(fromSeq), len(slice))
	}
	for i := range slice {
		if !bytesEqual(slice[i], fromSeq[i]) {
			t.Errorf("blob[%d] differs between CurrentTxs and CurrentTxsSeq", i)
		}
	}

	// Early termination must stop iteration without panicking.
	var seen int
	for range ol.CurrentTxsSeq() {
		seen++
		if seen == 1 {
			break
		}
	...
	if seen != 1 {
		t.Errorf("early break: seen=%d, want 1", seen)
	}
}
