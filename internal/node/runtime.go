...
func rpcFastSyncMetrics(snapshot adaptor.FastSyncMetrics) types.FastSyncMetrics {
	return types.FastSyncMetrics{
		...
		ReplayPipelinePivotNodesExamined:     snapshot.ReplayPipelinePivotNodesExamined,
		ReplayPipelinePivotEqualSubtrees:     snapshot.ReplayPipelinePivotEqualSubtrees,
		ReplayPipelinePivotMissingNodes:      snapshot.ReplayPipelinePivotMissingNodes,
		ReplayPipelinePivotStateDownloaded:   snapshot.ReplayPipelinePivotStateDownloaded,
	...
	if !r.standalone {
		...
		if router := r.consensus.Router; router != nil {
			...
			r.services.FastSyncMetrics = func() types.FastSyncMetrics {
				return rpcFastSyncMetrics(router.FastSyncMetrics())
			}
		}
