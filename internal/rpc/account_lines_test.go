...
func (m *mockAccountLinesLedgerService) GetAccountNFTs(_ context.Context, account string, ledgerIndex string, limit uint32, _ string) (*types.AccountNFTsResult, error) {
	return nil, errors.New("not implemented")
}
...
func TestAccountLinesPagination(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectError:  true,
			expectedCode: rpcerrors.RpcINVALID_PARAMS, // Invalid parameters error for negative limit
		},
		{
			...
			expectError:  true,
			expectedCode: rpcerrors.RpcINTERNAL,
		},
		{
