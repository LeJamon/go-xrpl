...
type issue1902NFTMarkerService struct {
	*mockNFTOffersLedgerService
	entries   map[[32]byte]*types.LedgerEntryResult
	buyCalls  int
	sellCalls int
}

func (m *issue1902NFTMarkerService) GetLedgerEntry(_ context.Context, key [32]byte, _ string) (*types.LedgerEntryResult, error) {
...
func (m *issue1902NFTMarkerService) GetNFTBuyOffers(_ context.Context, _ [32]byte, _ string, _ uint32, _ string) (*types.NFTOffersResult, error) {
	m.buyCalls++
	return &types.NFTOffersResult{Offers: []types.NFTOfferInfo{}}, nil
}

func (m *issue1902NFTMarkerService) GetNFTSellOffers(_ context.Context, _ [32]byte, _ string, _ uint32, _ string) (*types.NFTOffersResult, error) {
	m.sellCalls++
	return &types.NFTOffersResult{Offers: []types.NFTOfferInfo{}}, nil
}

func issue1902NFTOfferEntry(t *testing.T, tokenID [32]byte, sell bool) ([32]byte, *types.LedgerEntryResult) {
	t.Helper()
	_, accountBytes, err := addresscodec.DecodeClassicAddressToAccountID(ownerForAggregatePriceTest)
	require.NoError(t, err)
	var ownerID [20]byte
	copy(ownerID[:], accountBytes)
	flags := uint32(0)
	if sell {
		flags = ledgerentry.LsfSellNFToken
	}
	node, err := state.SerializeNFTokenOffer(ownerID, tokenID, "1000000", flags, 1, 1, "", nil)
	require.NoError(t, err)
	markerKey := [32]byte{0xA5}
	return markerKey, &types.LedgerEntryResult{Node: node}
}

func TestIssue1902NFTOfferMarkerSideValidation(t *testing.T) {
	const nftIDHex = "00081388DC1AB4E7C57F8067A3AB15BEA8B0F1A0DE14678200000099000001F4"
	nftBytes, err := hex.DecodeString(nftIDHex)
	require.NoError(t, err)
	var nftID [32]byte
	copy(nftID[:], nftBytes)
	...
	tests := []struct {
		...
		{name: "buy marker accepted by buy query", sell: false, querySell: false},
		{name: "sell marker rejected by buy query", sell: true, querySell: false},
		{name: "sell marker accepted by sell query", sell: true, querySell: true},
		{name: "buy marker rejected by sell query", sell: false, querySell: true},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			markerKey, entry := issue1902NFTOfferEntry(t, nftID, test.sell)
			service := &issue1902NFTMarkerService{
				mockNFTOffersLedgerService: newMockNFTOffersLedgerService(),
				entries: map[[32]byte]*types.LedgerEntryResult{
					markerKey:                  entry,
					keylet.NFTBuys(nftID).Key:  {Node: []byte("directory")},
					keylet.NFTSells(nftID).Key: {Node: []byte("directory")},
				},
			}
			ctx := &types.RpcContext{
				Context:    context.Background(),
			...
			params, err := json.Marshal(map[string]any{
				"nft_id": nftIDHex,
				"marker": hex.EncodeToString(markerKey[:]),
			})
			require.NoError(t, err)
			...
			if test.querySell {
				result, rpcErr = (&handlers.NftSellOffersMethod{}).Handle(ctx, params)
			} else {
				result, rpcErr = (&handlers.NftBuyOffersMethod{}).Handle(ctx, params)
			}
			if test.sell == test.querySell {
				require.Nil(t, rpcErr)
				require.NotNil(t, result)
				if test.querySell {
					assert.Equal(t, 1, service.sellCalls)
				} else {
					assert.Equal(t, 1, service.buyCalls)
				}
			} else {
				assert.Nil(t, result)
				require.NotNil(t, rpcErr)
				...
				assert.Equal(t, 0, service.buyCalls+service.sellCalls)
			}
		})
	}
}
...
func TestIssue1902NFTOfferMarkerLookupErrorsFallThroughToFetcher(t *testing.T) {
	// Existing ledger-service implementations surface non-not-found read errors
	// through the fetcher. Keep that error mapping intact while side validation is
	// available for a successfully decoded marker entry.
