...
// (incorrectly mapped to an amendment-error slot); we emit the rippled code
// directly so handlers like book_offers can report "feature accepted by the
// request shape but not honoured by this server" with byte-compatible output.
