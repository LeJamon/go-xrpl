...
func (m *DepositAuthorizedMethod) Handle(ctx *types.RpcContext, params json.RawMessage) (any, *rpcerrors.RpcError) {
	...
	if !hasSource {
		return nil, rpcerrors.RpcErrorMissingField("source_account")
	}
	sourceAccount, validSource := rawJSONString(sourceRaw)
	if !validSource {
		return nil, rpcerrors.RpcErrorExpectedField("source_account", "a string")
	}
	...
	if !hasDestination {
		return nil, rpcerrors.RpcErrorMissingField("destination_account")
	}
	destinationAccount, validDestination := rawJSONString(destinationRaw)
	if !validDestination {
		return nil, rpcerrors.RpcErrorExpectedField("destination_account", "a string")
	}
	...
	if _, err := ctx.Services.Ledger().GetAccountInfo(ctx.Context, sourceAccount, ledgerIndex); err != nil {
		if errors.Is(err, svcerr.ErrAccountNotFound) {
			return nil, rpcerrors.RpcErrorSrcActNotFound("Source account not found.").WithExtra(lookupExtra)
		}
	...
	if _, err := ctx.Services.Ledger().GetAccountInfo(ctx.Context, destinationAccount, ledgerIndex); err != nil {
		if errors.Is(err, svcerr.ErrAccountNotFound) {
			return nil, rpcerrors.RpcErrorDstActNotFound("Destination account not found.").WithExtra(lookupExtra)
		}
	...
	if err != nil {
		...
		case errors.Is(err, svcerr.ErrBadCredentials):
			...
			return nil, rpcerrors.RpcErrorBadCredentials(detail).WithExtra(lookupExtra)
		}
...
func parseCredentialsFormat(raw json.RawMessage) ([]string, *rpcerrors.RpcError) {
	var entries []json.RawMessage
	if err := json.Unmarshal(raw, &entries); err != nil || len(entries) == 0 {
		return nil, rpcerrors.RpcErrorExpectedField("credentials",
			"is non-empty array of CredentialID(hash256)")
	}

	if len(entries) > maxCredentialsArraySize {
		return nil, rpcerrors.RpcErrorExpectedField("credentials", "array too long")
	}
