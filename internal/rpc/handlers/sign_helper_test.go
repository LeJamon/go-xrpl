...
func TestSigningRequestUnmarshal(t *testing.T) {
	...
	if string(request.Seed) != `"seed"` || string(request.KeyType) != `"ed25519"` || !request.Offline.value || request.SignatureTarget != counterpartySignatureField || len(request.TxJson) == 0 {
