...
func addServerDiagnostics(info map[string]any, services *types.ServiceGraph) {
	...
	if services != nil && services.FastSyncMetrics() != nil {
		...
		counters["fast_sync"] = map[string]any{
			...
			"replay_pipeline_pivot_nodes_examined":           strconv.FormatUint(metrics.ReplayPipelinePivotNodesExamined, 10),
			"replay_pipeline_pivot_equal_subtrees_skipped":   strconv.FormatUint(metrics.ReplayPipelinePivotEqualSubtrees, 10),
			"replay_pipeline_pivot_missing_nodes_found":      strconv.FormatUint(metrics.ReplayPipelinePivotMissingNodes, 10),
			"replay_pipeline_pivot_state_nodes_downloaded":   strconv.FormatUint(metrics.ReplayPipelinePivotStateDownloaded, 10),
