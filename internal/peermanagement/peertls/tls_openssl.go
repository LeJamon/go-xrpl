...
func (c *conn) handshakeLoop(ctx context.Context) error {
	for {
		...
		if flushErr := c.flushStepOutputLocked(out, drainErr); flushErr != nil {
			return flushErr
		}
		if err == nil {
			return nil
...
// flushStepOutputLocked queues output drained with an SSL operation, then
// flushes it through the single raw-write path. The caller holds outMu.
func (c *conn) flushStepOutputLocked(out []byte, drainErr error) error {
