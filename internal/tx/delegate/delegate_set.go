...
func DeleteDelegate(ctx *tx.ApplyContext, delegateKey keylet.Keylet, existingData []byte) ter.Result {
	...
	if existingEntry.Account == ctx.AccountID {
		if result := tx.DecreaseOwnerCountForObject(ctx, existingEntry.Account, ctx.Account, existingData, "Sponsor", 1); result != ter.TesSUCCESS {
			return result
		}
	} else {
		...
		if result := tx.DecreaseOwnerCountForObject(ctx, existingEntry.Account, account, existingData, "Sponsor", 1); result != ter.TesSUCCESS {
			return result
		}
	}
