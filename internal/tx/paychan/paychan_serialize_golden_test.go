...
// The vectors were captured from the pre-refactor path (serializePayChannel →
// ParsePayChannel → SerializePayChannelFromData) and proven byte-identical; any
// change is a state fork.
func TestPayChannelSerialize_Golden(t *testing.T) {
	minimal := &PaymentChannelCreate{SettleDelay: 3600}
	...
	cases := []struct {
		...
		{name: "minimal", tx: minimal, amount: 100000000, ownerNd: 2, destNd: 4, hasDest: true,
			wantHex: "1100782200000000202700000E10340000000000000002390000000000000004614000000005F5E10062400000000000000081140102030405060708090A0B0C0D0E0F10111213148314A1A2A3A4A5A6A7A8A9AAABACADAEAFB0B1B2B3B4"},
		...
		{name: "no_destnode", tx: minimal, amount: 5000000, ownerNd: 1, hasDest: false,
			wantHex: "1100782200000000202700000E103400000000000000016140000000004C4B4062400000000000000081140102030405060708090A0B0C0D0E0F10111213148314A1A2A3A4A5A6A7A8A9AAABACADAEAFB0B1B2B3B4"},
	}
