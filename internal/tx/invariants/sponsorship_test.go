...
func sponsorshipTestRippleStateWithEmptySponsor(t *testing.T) []byte {
	t.Helper()
	line := &entry.RippleState{}
	if err := line.Decode(sponsorshipTestRippleState(t, false, false)); err != nil {
		t.Fatal(err)
	}
	line.SetHighSponsor("")
	data, err := line.Encode()
	if err != nil {
		t.Fatal(err)
	}
	return data
...
func TestSponsorshipOwnerCountsUsesRippleStateFieldPresence(t *testing.T) {
	object := sponsorshipTestRippleStateWithEmptySponsor(t)
	valid := sponsorshipTestEntries(t, entry.TypeRippleState, object, 1, 1)
	if violation := checkSponsorshipOwnerCounts(valid); violation != nil {
		t.Fatalf("explicit empty HighSponsor field must count as sponsored: %v", violation)
	}
}
