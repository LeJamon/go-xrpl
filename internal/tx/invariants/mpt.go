...
type mptTransferChange struct {
	before *state.MPTokenData
	after  *state.MPTokenData
}

type invariantMPTView struct {
	view  ReadView
	rules *amendment.Rules
}

func (v invariantMPTView) Read(k keylet.Keylet) ([]byte, error) { return v.view.Read(k) }

func (v invariantMPTView) Exists(k keylet.Keylet) (bool, error) { return v.view.Exists(k) }

func (v invariantMPTView) Insert(keylet.Keylet, []byte) error {
	return fmt.Errorf("invariant view is read-only")
}

func (v invariantMPTView) Update(keylet.Keylet, []byte) error {
	return fmt.Errorf("invariant view is read-only")
}

func (v invariantMPTView) Erase(keylet.Keylet) error {
	return fmt.Errorf("invariant view is read-only")
}

func (v invariantMPTView) Rules() *amendment.Rules { return v.rules }
...
func checkLoanDefaultMPTTransfer(
	...
	if rules == nil || view == nil ||
		(!rules.Enabled(amendment.FeatureMPTokensV2) && !rules.Enabled(amendment.FeatureFixCleanup3_4_0)) {
		return nil
	}
	if _, ok := loanManageDefaultFields(tx); !ok {
		return nil
	}
	...
	for _, e := range entries {
		...
		if e.Before != nil {
			var err error
			before, err = state.ParseMPToken(e.Before)
			if err != nil {
				return &InvariantViolation{
					Name:    "ValidMPTTransfer",
					Message: fmt.Sprintf("could not parse MPToken before state: %v", err),
				}
			}
		}
		after, err := state.ParseMPToken(e.After)
		if err != nil {
			return &InvariantViolation{
				Name:    "ValidMPTTransfer",
				Message: fmt.Sprintf("could not parse MPToken after state: %v", err),
			}
		}
		if before != nil &&
			(before.MPTokenIssuanceID != after.MPTokenIssuanceID || before.Account != after.Account) {
			return &InvariantViolation{
				Name:    "ValidMPTTransfer",
				Message: "MPToken issuance or holder changed",
			}
		}
		if before != nil && before.MPTAmount == after.MPTAmount {
			continue
		}
		if before == nil && after.MPTAmount == 0 {
			continue
		}
		failedMPTChange = true
		id := after.MPTokenIssuanceID
		if changes[id] == nil {
			changes[id] = make(map[[20]byte]mptTransferChange)
		}
		changes[id][after.Account] = mptTransferChange{before: before, after: after}
	}

	if rules.Enabled(amendment.FeatureFixCleanup3_4_0) && result != TesSUCCESS && failedMPTChange {
		return &InvariantViolation{
			Name:    "ValidMPTTransfer",
			Message: "MPToken balance changed on failed transaction",
		}
	}
	if len(changes) == 0 {
		return nil
	...
	exemption := findLoanDefaultFreezeExemption(tx, view, rules)
	freezeView := invariantMPTView{view: view, rules: rules}
	for id, holders := range changes {
		issuanceData, err := view.Read(keylet.MPTIssuance(id))
		if err != nil {
			return &InvariantViolation{
				Name:    "ValidMPTTransfer",
				Message: fmt.Sprintf("could not read MPTokenIssuance: %v", err),
			}
		}
		if issuanceData == nil {
			return &InvariantViolation{
				Name:    "ValidMPTTransfer",
				Message: "MPToken transfer references a missing issuance",
			}
		}
		...
		if err != nil {
			return &InvariantViolation{
				Name:    "ValidMPTTransfer",
				Message: fmt.Sprintf("could not parse MPTokenIssuance: %v", err),
			}
		}

		senders, receivers := 0, 0
		invalidTransfer := false
		for account, change := range holders {
			beforeAmount := uint64(0)
			...
			exempt := loanDefaultMPTFreezeExempt(exemption, id, account)
			...
			if authErr != nil {
				return &InvariantViolation{
					Name:    "ValidMPTTransfer",
					Message: fmt.Sprintf("could not check MPToken authorization: %v", authErr),
				}
			}
			if !authorized {
				invalidTransfer = true
		...
		if (invalidTransfer || issuance.Flags&entry.LsfMPTCanTransfer == 0) && senders > 0 && receivers > 0 {
			return &InvariantViolation{
				Name:    "ValidMPTTransfer",
				Message: "invalid MPToken transfer",
			}
		}
	}
	return nil
...
func loanDefaultMPTAuthorized(
	...
	tokenData, err := view.Read(keylet.MPTokenByID(id, account))
	if err != nil {
		return false, err
	}
