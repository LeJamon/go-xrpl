...
func (s *SponsorshipSet) create(
	...
	if result := tx.CheckReserve(ctx, s.GetCommon(), sponsorID, sponsor, balanceAfterFee, tx.ReserveAdjustment{OwnerCountDelta: 1}, ter.TecUNFUNDED); result != ter.TesSUCCESS {
		return result
	}
	...
	reserveSponsor, result := tx.IncreaseOwnerCount(ctx, s.GetCommon(), sponsorID, sponsor, 1)
	if result != ter.TesSUCCESS {
		return result
	}
	if reserveSponsor != "" {
...
func (s *SponsorshipSet) update(
	...
	if s.FeeAmountDelta != nil {
		...
		if result := tx.CheckReserve(ctx, s.GetCommon(), sponsorID, sponsor, sponsor.Balance, tx.ReserveAdjustment{}, ter.TecUNFUNDED); result != ter.TesSUCCESS {
			return result
		}
	}
	if s.MaxFee != nil {
...
func (s *SponsorshipSet) delete(
	...
	objectData, err := state.SerializeSponsorship(object)
	if err != nil {
		return ctx.Internal("SerializeSponsorship(delete)", err)
	}
	if result := tx.DecreaseOwnerCountForObject(ctx, sponsorID, sponsor, objectData, "Sponsor", 1); result != ter.TesSUCCESS {
		return result
	}
	if object.HasFeeAmount {
