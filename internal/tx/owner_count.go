...
// AdjustOwnerCountForObject adjusts the reserve counters for an owned object.
// A sponsored object changes both accounts' sponsorship counters as well as the
// owner's total OwnerCount.
func AdjustOwnerCountForObject(view LedgerView, accountID [20]byte, sponsorID *[20]byte, delta int) error {
	if sponsorID == nil {
		return AdjustOwnerCount(view, accountID, delta)
	}
	if delta == 0 {
		return nil
	}
	...
	if account == nil {
		return fmt.Errorf("failed to read account root: account does not exist")
	}
	sponsorKey := keylet.Account(*sponsorID)
	sponsor, err := state.ReadAccountRoot(view, *sponsorID)
	if err != nil {
		return fmt.Errorf("failed to read sponsor account root: %w", err)
	}
	if sponsor == nil {
		return fmt.Errorf("failed to read sponsor account root: account does not exist")
	}

	current := ownerCounts(account)
	sponsorCurrent := ownerCounts(sponsor)
	account.OwnerCount = confineOwnerCount(account.OwnerCount, delta)
	account.SponsoredOwnerCount = confineOwnerCount(account.SponsoredOwnerCount, delta)
	sponsor.SponsoringOwnerCount = confineOwnerCount(sponsor.SponsoringOwnerCount, delta)
	next := ownerCounts(account)
	sponsorNext := ownerCounts(sponsor)
	...
	if err != nil {
		return fmt.Errorf("failed to serialize sponsor account root: %w", err)
	}
	if hook, ok := view.(ownerCountsAdjuster); ok {
		hook.AdjustOwnerCounts(accountID, current, next)
		hook.AdjustOwnerCounts(*sponsorID, sponsorCurrent, sponsorNext)
	} else if hook, ok := view.(ownerCountAdjuster); ok {
		hook.AdjustOwnerCount(accountID, current.Owner, next.Owner)
	}
	if err := view.Update(sponsorKey, sponsorData); err != nil {
		return fmt.Errorf("failed to update sponsor account root: %w", err)
	}
	if err := view.Update(accountKey, accountData); err != nil {
		return fmt.Errorf("failed to update account root: %w", err)
	}
	return nil
}
