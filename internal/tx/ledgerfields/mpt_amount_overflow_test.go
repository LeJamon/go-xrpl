...
// generated streaming decoder: an MPT mantissa >= 2^63 must error rather than
...
func TestReadMPTAmount_RejectsOverMaxInt64(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			value, err := sr.readAmountAny()
			if tc.wantErr {
				if err == nil {
					t.Fatalf("expected out-of-range error for magnitude %d, got %#v", tc.magnitude, value)
				}
				return
			}
			if err != nil {
			...
			if !ok {
				t.Fatalf("decoded value has type %T, want map", value)
			}
