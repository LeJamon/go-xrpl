...
func TestSerializationStylesMatchRippledTag(t *testing.T) {
	...
	if len(tagged) != len(Specs) {
		t.Fatalf("rippled %s has %d ledger templates, spec has %d", rippledStyleTag, len(tagged), len(Specs))
	}
...
func gitShowTaggedFile(t *testing.T, repo, path string) []byte {
	t.Helper()
	cmd := exec.Command("git", "-C", repo, "show", rippledStyleTag+":"+path)
	out, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("git show rippled %s:%s: %v: %s", rippledStyleTag, path, err, out)
	}
	return out
}
