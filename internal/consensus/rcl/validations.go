...
// tip strictly newer than minSeq, ignoring whether we have local
// ancestry for it. This is the no-trie fallback for GetPreferred:
// when we are behind the network and don't have the network's tip in
// our local ledger store yet, the trie can't resolve ancestry and
// returns no tip. Each trusted (non-negUNL) validator's most-recent
// full validation contributes one vote for its (LedgerID,
// LedgerSeq); ties broken by higher seq then lexicographic ID for
// determinism.
//
// Returns (zero, 0, false) when no trusted validator has a
// validation past minSeq.
...
// lexLessLgrID returns true when a sorts before b in byte order. Used
// as a deterministic tie-breaker for PreferredFromValidations so two
// nodes with the same vote distribution land on the same hash.
