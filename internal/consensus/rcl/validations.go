...
// nothing. PreferredFromValidations votes purely by validator-tip
// popularity from byNode (each trusted non-negUNL validator's most
// recent full validation contributes one vote), with ties broken by
// higher seq then lexicographic ID for determinism.
//
// Used by the round-boundary preferred-LCL check (matches rippled's
// NetworkOPsImp::checkLastClosedLedger which combines getPreferredLCL
// with peer LCL counts; goxrpl's PeerReportedLedgers is the
// peer-counts equivalent and is consulted separately by checkLedger).
//
