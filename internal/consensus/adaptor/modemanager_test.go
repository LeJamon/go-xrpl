...
// behavior: in production, OperatingMode is promoted to Full by
// direct SetOperatingMode calls in router.go and
// adaptor.AdoptLedgerFromHeader, NOT through ModeManager's state
// machine. So m.mode stays at its initial value while
// adaptor.GetOperatingMode() returns Full. When a
// ModeChangedEvent{wrongLedger} fires, OnEvent MUST consult the
// adaptor's actual opMode and trigger the Full → Syncing
// transition — otherwise the engine drops to wrongLedger silently
// while opMode stays at Full and startRoundLocked keeps
// re-promoting us to ModeProposing.
