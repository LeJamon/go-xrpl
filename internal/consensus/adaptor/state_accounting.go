...
// (server_state_duration_us, initial_sync_duration_us).
type StateAccountingSnapshot struct {
	...
	// mode since the last transition. Exposed as the top-level
	...
	// occurred. Emitted as initial_sync_duration_us only when non-zero.
...
// server_info fields. Map keys are the lowercase operating-mode names.
...
// by the server_info JSON, ordered to match its iteration.
