...
func TestManifest_Revoked_WithEmptyEphemeralField_Rejected(t *testing.T) {
	...
	for _, field := range []string{"SigningPubKey", "Signature"} {
		t.Run(field, func(t *testing.T) {
			...
			if err == nil || !strings.Contains(err.Error(), "revoked manifest must not carry ephemeral fields") {
				t.Fatalf("Deserialize error = %v, want revoked ephemeral rejection", err)
			}
		})
	}
}
