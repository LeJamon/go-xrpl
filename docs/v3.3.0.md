...
no-CGO leaf builds retain a fail-closed MPT backend so those portable targets
remain safe; binarycodec, relational storage, keylet, ledger, and other packages
that reach addresscodec still require the native CGO shim. That artifact-level
capability distinction is not a missing protocol feature or a v3.3.0 parity
exception.
