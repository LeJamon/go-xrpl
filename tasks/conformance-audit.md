...
- Files reviewed (Phase 1):
  - internal/tx/engine.go — 0 findings. New `EngineConfig.EnforceLoadFee bool` (default false, additive). Doc comment is a non-obvious why citing rippled Transactor.cpp minimumFee→scaleFeeLoad / LoadFeeTrack.cpp:85.
  - internal/tx/preclaim.go — 0 blocking, 1 nit (N1 near-duplicate floor-branch body 179-212; N2 gating differs from rippled's always-on open-view floor but is observably equivalent via TxQ admission, documented). New checkFee `else if EnforceLoadFee && FeeTrack!=nil && GetLoadFactor()>LoadBase` branch enforces scaleFeeLoad floor + telINSUF_FEE_P, mirroring Transactor.cpp:278-290; runs before the fee==0 short-circuit, matching rippled ordering (floor at 278-290 precedes zero-fee tesSUCCESS at 292-293).
  - internal/testing/{env.go,env_submission.go} + internal/testing/conformance/runner.go — 0 findings (test harness, not protocol-bearing). LoadFeeTrack threading + txqLoadFeeLookup transcription of TxQ_test.cpp verified faithful: "clear queue failure (load)" setRemoteFee(5×)+reset (~3995-4009); "Queue full drop penalty" raiseLocalFee ×30 + lowerLocalFee loop (~4673-4685). submitViaTxQ drain-removal matches rippled (TxQ::accept only on close).
- Files cleanup-only (Phase 0 skipped Phase 1): none — Phase 1 ran (engine.go + preclaim.go protocol-bearing).
- Cleanup commit: none — Phase 2 was a no-op. Every PR-introduced comment is load-bearing: rippled / TxQ_test.cpp citations (protected) or non-obvious whys (EnforceLoadFee rationale, drain-removal rationale). No restated-next-line / banner / temporal / name-paraphrase cruft introduced.
- Minor (M1, pre-existing, NOT a blocker for this PR): production TxQ direct-apply/accept path internal/ledger/openledger/txqadapter.go:154 (`TxqAdapter.ApplyTransaction`) sets neither OpenLedger nor EnforceLoadFee, so it does NOT enforce the load-scaled floor that rippled applies on its OpenView (Transactor.cpp:278-290; TxQ::accept runs open_=true) — under elevated load goXRPL would apply underpriced queued txns rippled drops (telINSUF_FEE_P), a consensus-relevant txn-set divergence. Its sibling PreclaimTransaction (txqadapter.go:244) already sets OpenLedger:true. Out of scope of issue #590 (harness-only) and pre-existing (this PR is purely additive to production, flag defaults false → no regression). Recommend a dedicated follow-up to wire EnforceLoadFee:true (or OpenLedger:true) into ApplyTransaction + a load-spike production test.
- Notes: Static gates run locally — `just build` ✅, `just vet` ✅, `just lint` ✅ (0 issues). Tests delegated to CI per finalize policy (not re-run locally). Branch 30 commits behind origin/main at finalize (under 50 threshold; no rebase). verify skill N/A — change touches internal/tx + internal/testing only, no JSON/wire surface.
