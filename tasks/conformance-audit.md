...
## 2026-06-03 — PR #727 — feat/issue-279-rpc-error-codes
- Rippled SHA at review: 7527e35379 (rippled-3.0.0 oracle — base branch is v3.0.0, so the v3 oracle, NOT the default 2.x rippled/)
- PR URL: https://github.com/LeJamon/go-xrpl/pull/727
- Review comment: https://github.com/LeJamon/go-xrpl/pull/727#issuecomment-4613053813
- Phase 0: protocol-bearing (9 files under internal/rpc/) → Phase 1 ran.
- Files reviewed (Phase 1):
  - internal/rpc/handlers/ledger_entry.go — 1 BLOCKING-class (FIXED in-branch), 1 Minor. B-class: selector table mirrored only the `account_root` alias and omitted rippled's canonical `account` AccountRoot selector (ledger_entries.macro:148 → LedgerEntry.cpp:747-750; account_root is the appended alias at :755) → `{"account":"r..."}` rejected by goXRPL but resolved by rippled. Fixed by adding the `account` selector + test (commit fe3536a3). Minor: `nftoken_offer` is a goXRPL-only superset selector rippled rejects (deliberate documented back-compat alias; left as-is). Rest of table verified entry-by-entry against the macro; too-many-fields (:760-778), unknown-selector apiVersion branching (:814-822), numeric entryNotFound (:838,850), unexpectedLedgerType via 0x11 SLE header (:853-856), uppercase-hex node_binary (:858-866) all CONFIRMED. Hex-index selectors match rippled parseObjectID "hex string".
  - internal/rpc/handlers/helpers.go (InjectDeliveredAmount + toMetaMap) — 0 blocking, 1 Nit. type-set {Payment,CheckCash,AccountDelete} ∧ tesSUCCESS, snake_case delivered_amount, real→Amount→"unavailable", idempotent — all match DeliveredAmount.cpp:92-103,150,157. Nit/doc: close-time/ledger-index gate (:70-74, getLedgerIndex()>=4594095 || getCloseTime()>446000000s) omitted; Go comment claims it "always holds" (true only post-Feb-2014 — all live/standalone/simulate; diverges only on pre-2014 mainnet replay) and misattributes resolution-order cite as :128-160 (actual :50-78). CheckCash/fix1623 sub-gate acknowledged in-comment, correct for served ledgers. Comment-accuracy only.
  - internal/rpc/handlers/simulate.go — 0 findings. delivered_amount injected into meta only in non-binary branch; meta_blob untouched. Matches Simulate.cpp:267-290.
  - internal/rpc/handlers/vault_info.go — 0 findings. Correctly switched to RpcErrorEntryNotFoundBare (bare token), matching VaultInfo.cpp:99-103 (rippled 3.0.0 did NOT migrate VaultInfo to the numeric path).
  - internal/rpc/types/errors.go — 0 findings. RpcENTRY_NOT_FOUND=98 / RpcUNEXPECTED_LEDGER_TYPE=99 match ErrorCodes.h:161-162; tokens+messages match ErrorCodes.cpp:121-122; rpcLAST=99; contiguous after rpcDOMAIN_MALFORMED=97. Bare-vs-numeric split correct; "Entry not found." default matches.
  - internal/rpc/{helpers,ledger_entry,simulate}_test.go, internal/rpc/types/errors_test.go — 0 findings (enum/constructor/bare-token regression guards; rippled-cite test docs).
- Wire-shape verify pass: not driven live. Answered statically against the deterministic, unchanged emitter at server.go:715-725 + RpcError JSON tags (types/errors.go:24-27): non-bare → error_code(int)+error_message(string); bare → omits both; new codes 98/99 ride the same path, no field-type drift. Consistent with prior finalizations of this surface (PR #588).
- Review-fix commit (B-class, fixed in-branch per autonomous-fix mandate): fe3536a3 — "fix: add canonical `account` ledger_entry selector for AccountRoot"
- Cleanup commit: none — Phase 2 no-op. Scanned every PR-added/rewritten comment across all 9 files; each is a rippled/<path>:<line> cite or a genuine non-obvious why (idempotency, 0x11 SLE-header encoding, map-vs-*tx.Metadata duality, "no hex fallback"). No banners/Step-N/temporal/restated-next-line cruft. Matches PR #588 outcome on this surface.
- Files cleanup-only (Phase 0 skipped Phase 1): none — Phase 1 ran.
- Local gates: build / vet / lint (changed pkgs, 0 issues) green post-fix. Tests delegated to CI per finalize policy. Branch 0 commits behind base origin/v3.0.0 at finalize (20 behind main, but main is not the base — no rebase). Worked in existing worktree goXRPL-worktrees/issue-279.
- Notes: v3.0.0 PR — reviewed against rippled-3.0.0@7527e35379 per the rippled-v3-oracle convention. Appended to this branch's (v3.0.0-lineage) copy and committed separately; the main-checkout canonical copy is bg-isolation-guarded (could not edit) — block also saved to the job dir. May append-collide with main's copy on eventual merge — keep all blocks.
