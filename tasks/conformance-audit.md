...
## 2026-06-12 — PR #906 — refactor/issue-876-config-keys
- Rippled SHA at review: 1e89286a92
- PR URL: https://github.com/LeJamon/go-xrpl/pull/906
- Review comment: https://github.com/LeJamon/go-xrpl/pull/906#issuecomment-4691289019
- Phase 0: protocol-bearing trigger files present (internal/consensus/adaptor/, internal/ledger/service/, internal/rpc/websocket.go) → Phase 1 ran. Scope extended into config/ (bulk of the diff) because it mirrors rippled Config.cpp / TxQ.cpp / ValidatorList.cpp, even though config/ is not in the Phase-0 trigger list.
- Files reviewed (Phase 1) — 0 Blocking, APPROVE:
  - internal/ledger/service/txq_config.go (+ _test) — 1 Minor (M1: `>0` override guard makes an explicit `0` unreachable for retry_sequence_percent / normal+slow percent / minimum_last_ledger_buffer; rippled BasicConfig.set overrides on key-presence incl. 0, TxQ.cpp:1919-1976). All 13 keys map correctly; percent clamps [0,1000]/[0,100] match TxQ.cpp:1962-1973; max-below-min hard error incl. 0=unlimited matches TxQ.cpp:1931-1951; defaults 32/256/128000 match TxQ::Setup (TxQ.h:64,100-109). Wiring live to production (server.go→TxQConfigFromTuning→service.go:358-360).
  - internal/consensus/adaptor/startup.go (+ _test) — 0 Blocking, 1 Nit (N1: pre-existing "clamps to [100,1000]" comment is actually a reject; not PR-introduced, left for follow-up). [voting]→fee-vote now LIVE (was hardcoded); defaults 10 / 10_000_000 / 2_000_000 drops match Config.h:68-74. server_domain→WithServerDomain→Handshake.cpp:210-211; [overlay] public_ip→WithPublicIP→Handshake.cpp:213-217. validation_archive absent-section→Enabled=false gate (issue #2 docs/code now consistent).
  - internal/rpc/websocket.go (+ _test) — 0 findings. websocket_ping_frequency→SetPingInterval→ping ticker; seconds units + 0→default(30s) fallback. goXRPL-native knob (no rippled operator key).
  - config/ (config.go, validation.go, misc.go, peer.go, validators.go, loader.go, typed_fields.go, database.go, voting.go, logging.go; defaults.go + diagnostics.go deleted) — 0 Blocking. Minors: M2 fetch_depth unset default MaxInt32 vs rippled 1e9 (Config.h:208; cosmetic, both unlimited); M3 sqlite temp_store unset default = memory vs rippled file (DatabaseCon.cpp:130; PRE-EXISTING, non-consensus scratch storage) + the PR's new database.go doc comment contradicted the code (fixed in cleanup). Correct: validation_seed+validator_token mutual-exclusion (Config.cpp:636-639); relay case-insensitive EqualFold over {all,trusted,drop_untrusted} (Config.cpp:608-633); network_id main/testnet/devnet→0/1/2 case-sensitive (Config.cpp:525-532); ParseValidatorsTxt first-token + threshold error propagation (ValidatorList.cpp:145-155); validators_file precedence + relative resolution (Config.cpp:903-942); validatePortString strconv.Atoi rejects empty/garbage/0; ALL-errors-at-once via errors.Join; zero-value reconciliation collapsed to a single source of truth.
  - internal/cli/server.go, storage/relationaldb/sqlite/{repository_manager.go, settings_test.go} — node_db cache_size (entries) / cache_age (minutes→chrono::minutes, DatabaseNodeImp.h:75) + [sqlite] PRAGMAs reach the live DB; safety_level high/low maps to DatabaseCon.cpp:135-140.
- Deletion safety: loader is LENIENT (viper default, no ErrorUnused/UnmarshalExact) → removed TOML keys (workers/ssl_verify/[crawl]/[vl]/[perf]/rpc_startup/path_search*/node_seed/import_db/zero_basefee/…) do NOT break existing operator configs (silently ignored). Every removed Go symbol (defaults.go/diagnostics.go + ~50 getters + loader surface) has 0 production references repo-wide. All 4 shipped .toml + the generate.go template are clean of removed keys. SIGHUP path works with ReloadConfig gone.
- Open follow-ups (Minor/Nit, non-gating, surfaced in the PR comment): M1 explicit-0 TxQ keys; M2 fetch_depth default sentinel; M3 align sqlite temp_store default to rippled `file`; M4 docs/operating.md still lists removed keys; N1 startup.go "clamps" comment. RESOLVED in commit 1ce0e655 (post-finalize, at user request): M1 → TxQ keys are now `*int` (presence-based override, explicit 0 honoured) + new TestTxQConfigFromTuning_ExplicitZeroOverridesDefault; M2 → GetFetchDepth unset returns 1e9 (const defaultFetchDepth); M3 → temp_store default "file" (TestDefaultSettingsUnchanged asserts only journal_mode/synchronous, so it did NOT flip — the earlier "flips the pin-test" worry was wrong); M4 → removed-key rows stripped from docs/operating.md; N1 → comment reworded to "rejects". N2 (unicode-whitespace tokenizer) and N3 (silent key drop) intentionally left — Go strictly more lenient / release-note item. config+service+sqlite+cli+adaptor+txq tests + build/vet/lint all green.
- Wire-shape verify pass: SKIPPED (justified) — the only internal/rpc/ change is websocket.go ping cadence (a timer, no JSON/protobuf field), and nothing under internal/rpc/handlers/ or internal/peermanagement/; no field-type-drift surface. Ping wiring verified statically.
- Files cleanup-only (Phase 0 skipped Phase 1): none — Phase 1 ran.
- Cleanup commit: bec949fe (config/database.go: corrected the SQLite default-settings doc comment — it claimed unset temp_store=file but applyPragmas defaults an unset temp_store to memory; +2/-3). All other PR-introduced comments kept — load-bearing (optional-key "0 = use default" annotations, rippled cites incl. TxQ.cpp:1930-1951, handshake-header wiring why, decoder-hook necessity, pebble-vs-config tuning distinction). validation.go numbered-step comments are a pre-existing convention (renumbered only) — out of scope.
- Notes: Branch 19 commits behind origin/main at finalize (< 50 threshold, no rebase). Worked in existing worktree goXRPL-worktrees/issue-876, clean tree throughout. Static gates green before and after both phases: just build, just vet, just lint (0 issues). Tests delegated to CI per finalize policy. Phase 1 made zero code edits (all Minors left as PR-comment findings). Reviewed via 5 parallel conformance subagents (TxQ / Config.cpp / validators / wiring / deletion-safety).
