...
- Notes: Tight conformance PR closing the four #503 audit gaps (manifest strict canonicality, ed25519 canonicality gate visibility, KeyType enum reordering, ed25519 secure_erase). Zero blockers — all four claimed fixes are correctly anchored to rippled. The two Minors are missing-test-coverage gaps that should land as a follow-up; both are reusable from existing helpers (verify_test.go::flipSToHighS for M1; ed25519Order constant arithmetic for M2). N1 deferred (comment-fidelity, not substance).
