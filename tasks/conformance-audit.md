...
## 2026-06-12 — PR #914 — refactor/issue-882-txq-divergences
- Rippled SHA at review: 1e89286a92
- PR URL: https://github.com/LeJamon/go-xrpl/pull/914
- Review comment: https://github.com/LeJamon/go-xrpl/pull/914#issuecomment-4691975043
- Phase 0: protocol-bearing files present (internal/tx/engine.go, internal/ledger/openledger/txqadapter.go; internal/txq/ is protocol-bearing on substance — mirrors rippled app/misc/detail/TxQ.cpp) → Phase 1 ran. Prior audits consulted as context: engine.go + txqadapter.go (PR #647, 0 findings — the EnforceLoadFee infra this PR's H1 builds on); txq.go (PR #884, 0 findings). rippled unmoved since (still 1e89286a92), but a full review was still performed — substantial TxQ admission rewrite (+540/-216).
- Verification method: 4 parallel go-xrpl-engineer subagents, each reading rippled TxQ.cpp in full + the relevant makeTxConsequences overrides; the byFee index-fixup math and the fee.go zero-base-fee/median claims were re-traced by hand.
- Files reviewed (Phase 1) — 0 blocking, 1 Minor (pre-existing), 2 Nit. APPROVE:
  - internal/txq/apply.go — H1 (preflight-first TxQ.cpp:743-745, always-preclaim 1167-1170, direct-apply-no-fallthrough 1711-1746, IsTec≡likelyToClaimFee), H2 (getNextQueuableSeq==txSeqProx else telCAN_NOT_QUEUE 1031-1040; nextQueuableSeqImpl port 1622-1666), H3 (new-tx + preceding-tx sandbox failure → nil fall-through 1216-1224 / 592-596), M1 (computeConsequences SendMax-else-Amount + Escrow/PayChanCreate/PayChanFund/XChainCommit — every case has a real makeTxConsequences override; fixes prior UNDER-counting), M3 (clear range [acctSeqProx,seqProxy) 536-542 + FirstRelevant gate 1198-1200). ALL FAITHFUL.
  - internal/txq/accept.go — M2 faithful: ticket-inclusive rbegin drop (1541), `endIter!=candidateIter` guard (1552-1554), byFee index fixup `*idx--` traced over 3 layouts == `++candidateIter` (1555), defensive eraseAndAdvance removeByFee branch, snapshot hoist (1447, const under q.mu).
  - internal/txq/fee.go — median collapse behaviour-preserving (== 160-162); zero-base-fee comment accurate (calculateDefaultBaseFee = Transactor::calculateBaseFee reference fee, applySteps.cpp:432-436; contextual==0 ⟹ reference==0 ⟹ mod collapses to 1, TxQ.cpp:47-52). 0 findings.
  - internal/txq/candidate.go — RelevantSortedCandidates/FirstRelevant = lower_bound(acctSeqProx) (818); SeqProxy.Less == operator<. 0 findings.
  - internal/txq/txq.go — GetMetrics maxSize value-copy (defensive); bytes.Compare ≡ deleted compareHashes; dead TxQFull() accessor removed (0 refs). 0 findings.
  - internal/txq/process.go — dead GetConfig removed (0 refs). 0 findings.
  - internal/ledger/openledger/txqadapter.go — PreflightTransaction added; preclaim OpenLedger:true→EnforceLoadFee:true correct (enforces Transactor::checkFee load-scaled floor 248-290, NOT open-ledger escalation which is the TxQ's own feeLevel decision). 0 findings.
  - internal/tx/engine.go — exported Preflight() delegates to internal preflight, no behavioural change. 0 findings.
  - internal/testing/env_submission.go — testTxQApplyContext.PreflightTransaction mirrors ApplyTransaction's EngineConfig. 0 findings.
  - internal/txq/admission_apply_test.go (new) + sandbox_test.go/txq_test.go — H1/H2/M2 + tryClearAccountQueue fall-through pins; read for intent, not run (CI).
- M-pre1 (Minor, PRE-EXISTING — present on merge-base 23a56ac4, untouched by the diff): apply.go:318-322 `goesAtFront` stale-predecessor clause forces the front-of-queue branch when the immediate predecessor is a stale (seq<acctSeq) entry → returns terPRE_SEQ where rippled (TxQ.cpp:1019-1041) takes the after-entries branch and returns telCAN_NOT_QUEUE. Reproducer: acctSeq=5, queue {seq3 stale, seq8}, new seq6. The clause is not load-bearing (the seq==acctSeq case proceeds anyway via the after-entries equality). Recommended fix: delete the clause (Go's getNextQueuableSeq already yields the correct after-entries telCAN_NOT_QUEUE). Surfaced to user as an optional in-branch fix / follow-up; does NOT gate this PR.
- N1 (Nit, latent/inert): apply.go:397 preclaim-accept predicate `!result.IsTec()` omits the tapRETRY term carried by rippled isTecClaimHardFail — inert because no TxQ::apply caller sets tapRETRY (added only in the consensus applyTransaction() retry loop). Worth a one-line comment, not a code change.
- N2 (Nit, FIXED in-finalize, commit 88857b00): as-submitted branch FAILED golangci-lint (goimports) — apply.go:10 (paychan import out of order vs payment) + admission_apply_test.go:48 (method-column alignment). Pure gofmt reformat, zero semantic change, committed standalone so the cleanup commit stays comment-only.
- Files cleanup-only (Phase 0 skipped Phase 1): none — Phase 1 ran (all protocol-bearing).
- Cleanup commit: none — Phase 2 no-op. Every PR-introduced comment (production + test) is a rippled behaviour cite (TxQ.cpp / Payment.cpp / Escrow.cpp / PayChan.cpp / XChainBridge.cpp) or a non-obvious why (snapshot-const-under-lock, direct-apply-no-fallthrough, preclaim-view defaulting, getFeeLevelPaid modifier reasoning, maxSize defensive copy, byFee-index-fixup, test parity-pins). No restated-next-line / banner / temporal-"#882" / removed-code-stub cruft. Tightening the verbose-but-correct why-comments would be a content rewrite, not a janitorial removal (forbidden). The only in-finalize code change is the gofmt fix (88857b00).
- Wire-shape verify pass: N/A — no files under internal/rpc/handlers/ or internal/peermanagement/; diff is internal/txq + internal/ledger/openledger + internal/tx + internal/testing, no JSON/protobuf wire surface (Step-3.4 trigger did not fire).
- Notes: Branch 20 commits behind origin/main at finalize (under 50, no rebase), 1 code commit ahead (29098824) + the gofmt commit (88857b00). Worked in existing worktree goXRPL-worktrees/issue-882, clean tree throughout. Static gates green AFTER the gofmt fix: just build (rc=0), just vet (rc=0), just lint (0 issues) — note the AS-SUBMITTED branch failed lint (N2). Tests delegated to CI per finalize policy. Conformance not run (PR body documents 5 pre-existing TxQ-suite failures + an intra-test step shift in the already-failing unexpected_balance_change; suite pass count unchanged 32/37, informational).
- POST-FINALIZE (user-requested, commit d6731c8b): M-pre1 + N1 fixed in-branch. M-pre1 — deleted the `goesAtFront` stale-predecessor clause so a stale predecessor falls through to the after-entries branch (telCAN_NOT_QUEUE), matching rippled; added the why-comment guarding against re-introduction (cites TxQ.cpp:1019) + new `TestApply_H2_StalePredecessorGap` (negative-control verified: FAILS terPRE_SEQ −92 vs expected telCAN_NOT_QUEUE −392 on the pre-fix code, PASSES after). N1 — added a comment at the preclaim site documenting why the tec accept is unconditional (no TxQ.Apply caller sets tapRETRY). Re-verified green: just build, just vet, just lint (0 issues), just test-pkg ./internal/txq/... (all pass). Net +29/−3 (2 files). N2 already fixed at 88857b00. All review findings now resolved in-branch; nothing left as a follow-up.
