...
- Adjacent finding (M1 — Minor, PRE-EXISTING, OUT OF SCOPE, recommend follow-up): validation `cookie` field emitted as base-16 hex (`strconv.FormatUint(v.Cookie, 16)`, server.go:1424) but rippled emits base-10 decimal (`std::to_string(*cookie)`, NetworkOPs.cpp:2429). This is a BASE mismatch, not a case issue; line 1424 is NOT in this PR's diff. Does not gate #788. Fix candidate (separate PR): `FormatUint(v.Cookie, 10)`.
