...
## 2026-05-29 — PR #634 — fix/issue-575-rfc1751-tests
- Rippled SHA at review: 1e89286a92
- PR URL: https://github.com/LeJamon/go-xrpl/pull/634
- Review comment: https://github.com/LeJamon/go-xrpl/pull/634#issuecomment-4576259863
- Files reviewed (Phase 1):
  - crypto/rfc1751/rfc1751_test.go — 0 findings (test-only, 302 lines). Reviewed the new suite AND the rfc1751 impl it exercises against rippled RFC1751.cpp + Seed.cpp::seedAs1751 + KeyGeneration_test.cpp. extract/insert/standard/wsrch/btoe/etob are byte-for-byte ports; cross-impl vectors (BE6A670A…/74BA8389…) lifted verbatim from KeyGeneration_test.cpp:50-87; two Appendix-B canonical vectors; index-570 "YOU" quirk faithfully ported and now pinned by TestIndex570NotDecodable. 1 Nit only: Go length guards (16-byte) on KeyToEnglish/SeedToEnglish are stricter than rippled's guard-less getEnglishFromKey — additive defensive safety, no rippled-caller analog, not a defect. Coverage exceeds the oracle (rippled has no dedicated RFC1751 unit test, only the RPC-level KeyGeneration_test). 0 blocking.
- Files cleanup-only (Phase 0 skipped Phase 1): none — Phase 0 proceeded to Phase 1 (crypto/ is protocol-bearing).
- Cleanup commit: none — Phase 2 was a no-op. Every comment in the file is a rippled/RFC cite (RFC1751.cpp:433/446/371-376, Seed.cpp seedAs1751, KeyGeneration_test.cpp, RFC 1751 Appendix B) or a non-obvious why (index-570 quirk); no restated-next-line/banner/temporal/step-narration cruft to strip.
- Notes: Local build/vet/lint all green (full-module lint 0 issues; lint initially blocked by a concurrent golangci-lint lock from a parallel job, retried clean). Tests delegated to CI per finalize policy; randomized round-trip tests use fixed rand seeds 1/2/3 so CI is reproducible. verify skill N/A — crypto codec change, no JSON/wire surface.
