...
## 2026-06-10 — PR #868 — feat/issue-852-grpc-serving
- Rippled SHA at review: 1e89286a92
- PR URL: https://github.com/LeJamon/go-xrpl/pull/868
- Review comment: https://github.com/LeJamon/go-xrpl/pull/868#issuecomment-4674290866
- Phase 0: classifier prefix-list would skip (files under config/ + internal/cli/, neither in the protocol-bearing prefix set), BUT Phase 1 was run deliberately — this PR's config/lifecycle semantics are oracle-checked against rippled GRPCServer.cpp (port_grpc section, secure_gateway unspecified-IP rejection). Treated as protocol-adjacent.
- Files reviewed (Phase 1) — verdict APPROVE, 0 blocking / 0 minor / 0 nit:
  - config/config.go — 0 findings (GetGRPCPort disabled-by-default matches GRPCServer.cpp:323-333; rippled cite kept)
  - config/server.go — 0 findings (HasGRPC follows Has* idiom; validateProtocols grpc-combination check enforces rippled's structural dedicated-listener invariant)
  - internal/cli/grpc.go — 0 findings (secure_gateway unspecified-IP rejection mirrors GRPCServer.cpp:361-368; the parse-but-no-op is HONEST — go-xrpl gRPC service has zero Resource/Charge/isUnlimited/Role surface, so no per-request bypass exists to wire, unlike rippled clientIsUnlimited→usage.charge at :146-157)
  - internal/cli/server.go — 0 findings (lifecycle: ledgerService Start()ed :257 before grpc start :1046; bind error propagated via return :1048-1050 — fails loudly; Serve goroutine spawned only after net.Listen — no leak on bad bind; GracefulStop+hard-Stop fallback bounded by shared 30s ctx :1177-1210 — cannot hang; listenerErrCh buffer +2 correct)
  - Dependency-wiring: live *service.Service implements all six LedgerLookup methods; test stubLookup mirrors the same interface (compile-time assert) → TestGRPCServer_RoundTrip exercises the real wiring path
  - IP-binding default: GetBindAddress ":0" wildcard fallback is unreachable for grpc ports — config/loader.go:54 ValidateConfig→validatePorts→Validate rejects empty IP / zero port at load (rippled silently skips; go-xrpl loudly rejects; both refuse the wildcard bind)
  - Tests (config/grpc_test.go, internal/cli/grpc_test.go) — 0 findings (round-trip + reject-unspecified + disabled-by-default + protocol-combination pins all present)
