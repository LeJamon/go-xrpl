...
## 2026-06-12 — PR #909 — refactor/issue-880-misc-internal
- Rippled SHA at review: 1e89286a92
- PR URL: https://github.com/LeJamon/go-xrpl/pull/909
- Review comment: https://github.com/LeJamon/go-xrpl/pull/909#issuecomment-4691412624
- Phase 0: NO file on the mechanical protocol-bearing prefix list, but internal/validator/list/ (UNL blob + on-disk cache parsing) and internal/manifest/ (validator manifest verification) are XRPL wire formats where rippled is the oracle, so Phase 1 ran ON SUBSTANCE — same precedent as the internal/cli/ #787 entry. feetrack/observability/statecompare reviewed for behaviour-preservation.
- Files reviewed (Phase 1):
  - internal/validator/list/aggregator.go — 0 findings, 0 blocking. Off-lock cache writes verified RACE-FREE + COMPLETE flush coverage: queue-under-a.mu / flush-after-unlock; the `defer flushCacheWrites` + `defer mu.Unlock` LIFO pairs at all 5 sites (PublisherSnapshot/ApplyList/Tick/handleRevocation/TrustedValidators) run flush AFTER unlock; every writeCacheLocked/removeCacheLocked caller reaches a flush; per-publisher seq stamp drops superseded mutations regardless of flush interleave (latest seq wins via cacheWritten[pk]); a.mu and cacheWriteMu never held together (no lock-order deadlock). applyCachedCollection→ApplyCollection fold behaviour-equivalent (same supported-version/len/MaxSupportedBlobs guards + per-blob manifest fallback + any-ShouldRelay). Matches rippled logs-and-ignores cache-write (ValidatorList.cpp:388-395).
  - internal/validator/list/cache.go — 0 findings, 0 blocking. envelope unification byte-neutral: uint32→float64 refresh_interval both marshal {"refresh_interval":1440} (verified empirically), field order == old cachedEnvelope → on-disk bytes unchanged.
  - internal/validator/list/site_poller.go — 1 Minor + 1 Nit, 0 blocking. M1 (PRE-EXISTING, not introduced by this PR): recordFailure uses 30s ErrorRetryInterval for ALL failures incl. JSON-parse/missing-fields, while rippled applies 30s only on transport/HTTP-status (retry=true, ValidatorSite.cpp:558-560) and the FULL refresh interval on parse exceptions (retry=false, :615-620). N1: envelope doc claims "byte-compatible" via rippled field order but rippled order is manifest/version/public_key/... vs goXRPL manifest/public_key/...; names+types match (schema-compatible), order does not. SetNextRefresh-before-fetch matches onTimer (ValidatorSite.cpp:354-355→358); double-Start guard matches rippled timer-expiry guard (:171-172). recordFailure/UpdateSiteState overwrite the in-flight SetNextRefresh on completion → no RPC inconsistency introduced.
  - internal/validator/list/blob.go — 0 findings, 0 blocking. VerifyKeyTypeSignature dedup byte-equivalent to old verifyBlobSignature: secp256k1 .Validate delegates to .ValidateWithCanonicality(...,true) (secp256k1.go:225), ed25519 unchanged; rippled routes BOTH manifest+VL-blob through ripple::verify w/ mustBeFullyCanonical=true (Manifest.cpp:209/212→Sign.cpp:60; ValidatorList.cpp:1387; PublicKey.h:256). protocol.RippleEpochUnix=946684800 int64 (constants.go:30) == removed local const. No import cycle (manifest does not import validator/list).
  - internal/manifest/manifest.go — 0 findings, 0 blocking. VerifyKeyTypeSignature extraction (now exported) rippled-faithful; secp256k1 canonicality preserved.
  - internal/observability/sched_latency.go — 0 findings. Comment fix only: latencyEventMs→NotifyEvent wiring at :149-151 already exists; old "dormant/TODO" text was stale. Matches Application.cpp:134-135.
  - internal/feetrack/feetrack.go — 0 findings. max(a,b,c) builtin (uint32-preserving) == nested maxU32.
  - internal/statecompare/client.go — 0 findings (NON-protocol dev/replay tool). toHash32 length-check replaces silent copy truncation/zero-pad; ErrNotFound additive (no string-match callers on old "ledger %d not found").
  - test files (aggregator_test.go, site_poller_test.go [new, 3 cases]) — compile (vet/lint type-check tests); runtime pass/fail delegated to CI.
- Wire-shape verify pass: Step-3.4 path trigger NOT fired — no file under internal/rpc/handlers/ or internal/peermanagement/. Cache JSON byte-stability instead confirmed by a marshal experiment (uint32 vs float64 refresh_interval identical).
- Files cleanup-only (Phase 0 skipped Phase 1): none — Phase 1 ran on all protocol-bearing-in-substance files.
- Cleanup commit: c39c6e78 (manifest.go + blob.go: two "shared by/used by" cross-reference sentences on VerifyKeyTypeSignature/verifyBlobSignature trimmed — provenance not behaviour; restated visible delegation. +2/-5. rippled cites + secp256k1 canonicality rationale retained.)
- Notes: Branch 10 commits behind origin/main at finalize (under 50, no rebase). Worked in existing worktree goXRPL-worktrees/issue-880, clean tree throughout. Static gates green before+after both phases: just build (rc=0, recompiled validator/list), just vet (clean), just lint (0 issues). Tests delegated to CI per finalize policy. Phase 1 made ZERO code edits (M1 pre-existing, N1 cosmetic). Sub-agent rippled cross-checks (ValidatorSite onTimer; ValidatorList cache/sig) both CONFIRMED.
- Follow-up (post-review, commit d1138126, pushed): all four findings RESOLVED on request. M1 — recordFailure now takes a `retry` flag (transport/HTTP-status→30s error_retry_interval, parse/validation→full refresh interval) matching rippled onSiteFetch (ValidatorSite.cpp:551-620). N3 — site poller made restartable after Stop (fresh stop channel per run, `started` cleared on Stop, runLoop takes the stop chan) mirroring ValidatorSite::start rearm (:171-172,:198); added TestSitePoller_RestartAfterStop, double-Start no-op guard preserved. N1 — envelope doc corrected to "schema-compatible" (matching field names/types; field order differs from buildFileData and is not significant on a JSON-unmarshal read path), not "byte-compatible". N2 — distinct "unknown signing key type" error restored in verifyBlobSignature (lost when key-type dispatch moved to shared VerifyKeyTypeSignature). Plus a redundant composite-literal type dropped in aggregator_test. validator/list tests PASS under -race incl. the new restart test; fetch success path + trusted-set output unchanged. Static gates green (build/vet/lint 0 issues).
