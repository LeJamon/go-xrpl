...
- 1.2 IN PROGRESS — revive `exported` audit found 257 undocumented exported symbols across public pkgs (storage 166, shamap 44, config 29, small tail elsewhere). Tool: `go run github.com/mgechev/revive@v1.7.0 -config <jobtmp>/revive-exported.toml ./<pkg>/...`. Stutter-rename warnings (ED25519CryptoAlgorithm etc.) are NOT doc issues and are intentionally left (renaming = breaking API). Done so far: drops, protocol, crypto/common, crypto/multisign, codec/binarycodec/types (13 comments, build clean, revive missing-doc clean for those). Remaining: storage/*. (config done: 27 getter/IsZero comments. shamap done: 44 comments — leaf-node interface methods across 3 types, 2 caches, node-type/state/diff const blocks, error vars; build+revive clean.)
