...
- 1.2 IN PROGRESS — revive `exported` audit found 257 undocumented exported symbols across public pkgs (storage 166, shamap 44, config 29, small tail elsewhere). Tool: `go run github.com/mgechev/revive@v1.7.0 -config <jobtmp>/revive-exported.toml ./<pkg>/...`. Stutter-rename warnings (ED25519CryptoAlgorithm etc.) are NOT doc issues and are intentionally left (renaming = breaking API). Done so far: drops, protocol, crypto/common, crypto/multisign, codec/binarycodec/types (13 comments, build clean, revive missing-doc clean for those). Remaining storage backend: relationaldb/sqlite (74). (postgres DONE: 66 methods across 8 files, build+revive clean @ 9f3e1c44. NOTE: file reads here are intermittently contaminated — re-read per-file, trust revive as ground truth, Edit fails safe on bad signatures.) Earlier: relationaldb/postgres (60) — near-identical repository methods implementing documented relationaldb interfaces; concise one-line comments. (config 27 done. shamap 44 done. storage core done: nodestore 5 + relationaldb core 11 + kvstore/pebble 8 + kvstore/memorydb 8 = 32, build+revive clean.)
