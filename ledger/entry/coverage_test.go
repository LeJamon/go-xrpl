...
func TestGeneratedSLE_RoundTripAndAccessors(t *testing.T) {
	for name, fixture := range coverageFixtures {
		t.Run(name, func(t *testing.T) {
			...
			prevEqual := NewByName(name)
			if err := prevEqual.Decode(canonical); err != nil {
				t.Fatalf("Decode prev: %v", err)
			}
			prevEmpty := NewByName(name)
			prevWrong := NewByName("AccountRoot")
			if name == "AccountRoot" {
				prevWrong = NewByName("Offer")
			}
			...
			for cut := 0; cut < len(canonical); cut++ {
				_ = NewByName(name).Decode(canonical[:cut])
			}
			...
			err = NewByName(name).Decode(unknown)
			if err == nil {
...
// schema.Specs: every in-scope entry type needs a fixture, every fixture must set
...
func TestGeneratedSLE_FixtureCompleteness(t *testing.T) {
	...
	for _, e := range schema.Specs {
		known[e.Name] = true
	}
	for name := range coverageFixtures {
		if !known[name] {
			t.Errorf("coverage fixture %q has no matching schema.Specs entry", name)
		}
	}
}
