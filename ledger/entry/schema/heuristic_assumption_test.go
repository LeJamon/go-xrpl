...
// on: for every schema.Field whose Meta is MetaDefault, the generated
...
func TestEmitFinalFieldsSubsetOfEmitPreviousFields(t *testing.T) {
	genDir, ok := findGeneratedDir()
	if !ok {
		t.Fatal("ledger entry generated directory not found")
	}
...
// fileNameForEntry mirrors the snake() helper in cmd/entrygen:
...
// findGeneratedDir returns the directory containing the codegen output.
func findGeneratedDir() (string, bool) {
	_, file, _, ok := runtime.Caller(0)
	if !ok {
		return "", false
	}
	parent := filepath.Dir(filepath.Dir(file))
	if _, err := os.Stat(filepath.Join(parent, "models.go")); err != nil {
		return "", false
	}
	return parent, true
}
