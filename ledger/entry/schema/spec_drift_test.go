...
func TestSpecCoversRippledMacro(t *testing.T) {
	...
	if len(rippled) != len(Specs) {
		t.Fatalf("rippled v3.4.0-rc1 has %d ledger templates, schema has %d", len(rippled), len(Specs))
	}
	...
	if canonical != len(rippled) {
		t.Errorf("registry has %d canonical entries, rippled v3.4.0-rc1 has %d", canonical, len(rippled))
	}
}
...
func requireRippledMacro(t *testing.T) string {
	...
	for range 12 {
		candidate := filepath.Join(dir, "rippled-worktrees", "v3.4.0-rc1-oracle", "include", "xrpl", "protocol", "detail", "ledger_entries.macro")
	...
	t.Fatalf("required rippled v3.4.0-rc1 oracle not found from %s", file)
	return ""
}
