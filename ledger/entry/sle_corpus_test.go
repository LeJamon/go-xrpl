...
func decodeSLECorpusCase(t *testing.T, vector sleCorpusVector) sleCorpusCase {
	...
	entry := NewByName(vector.LedgerEntryType)
	if entry == nil {
...
func TestSLECorpusManifest(t *testing.T) {
	...
	registered := make([]string, 0, len(schema.Specs))
	for _, entrySpec := range schema.Specs {
		if NewByName(entrySpec.Name) == nil {
			t.Fatalf("%s has no typed constructor", entrySpec.Name)
		}
		registered = append(registered, entrySpec.Name)
	}
...
func TestSLECorpusFullVectorsCoverSpecs(t *testing.T) {
	byType := make(map[string]sleCorpusCase, len(schema.Specs))
	...
	for _, entrySpec := range schema.Specs {
		...
		for _, field := range entrySpec.Fields {
			...
			if _, ok := corpusCase.Fields[field.Name]; !ok && field.Style != schema.StyleDefault {
