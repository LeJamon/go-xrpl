...
func uint32FromJSON(value any) (uint32, error) {
	var val uint64
	switch v := value.(type) {
	case uint32:
		val = uint64(v)
	...
	case float64:
		if math.IsNaN(v) || math.IsInf(v, 0) || v < 0 || v > math.MaxUint32 || math.Trunc(v) != v {
			return 0, fmt.Errorf("value %v out of range for UInt32", v)
		}
		val = uint64(v)
	default:
		return 0, fmt.Errorf("unsupported type %T for UInt32", value)
	}
