...
func TestSTArrayToJson(t *testing.T) {
	...
	tt := []struct {
		...
		{
			name: "fail - binary parser read field error",
			malleate: func(t *testing.T) *serdes.BinaryParser {
				// Structurally valid 3-byte header with no matching field.
		...
		{
			name: "fail - malformed templated starray element",
			malleate: func(t *testing.T) *serdes.BinaryParser {
			...
			expectedErr: errors.New("Field 'Account' is required but missing."),
		},
	}
